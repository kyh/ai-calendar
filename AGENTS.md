...
There is no test suite and no CI workflow in this repo, so `pnpm verify` + `pnpm build` is the whole automated safety net. `format` is `oxfmt --check` (it fails, it does not rewrite); use `pnpm format:fix` to apply.

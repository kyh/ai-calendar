...
- **No `any`, no non-null `!`, no `as` casts** — enforced by `oxlint.config.ts` (ultracite `core` + `react` + `next` + `anti-slop`; every rule is an error, `no-await-in-loop` is the one deliberate override), not just documented. Prefer fixing code over `oxlint-disable` comments; when a rule is genuinely wrong for a line, disable that line with a `-- reason`. Parse at boundaries with zod (stream events, tool payloads, persisted storage).
